...
interface MediaProps {
	geometry: Plane; gl: GL; image: string; index: number; length: number; renderer: Renderer; scene: Transform;
	screen: ScreenSize; text: string; viewport: Viewport; bend: number; textColor: string; borderRadius?: number; font?: string;
}
class Media {
	extra = 0; geometry: Plane; gl: GL; image: string; index: number; length: number; renderer: Renderer; scene: Transform; screen: ScreenSize; text: string; viewport: Viewport; bend: number; textColor: string; borderRadius: number; font?: string;
	program!: Program; plane!: Mesh; title!: Title; scale!: number; padding!: number; width!: number; widthTotal!: number; x!: number; speed = 0; isBefore = false; isAfter = false;
	constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font }: MediaProps) {
		this.geometry = geometry; this.gl = gl; this.image = image; this.index = index; this.length = length; this.renderer = renderer; this.scene = scene;
		this.screen = screen; this.text = text; this.viewport = viewport; this.bend = bend; this.textColor = textColor; this.borderRadius = borderRadius; this.font = font;
	...
	createShader() {
        ...
        void main(){ vec2 ratio=vec2(min((uPlaneSizes.x/uPlaneSizes.y)/(uImageSizes.x/uImageSizes.y),1.0),min((uPlaneSizes.y/uPlaneSizes.x)/(uImageSizes.y/uImageSizes.x),1.0));
          ...
          float d=roundedBoxSDF(vUv-0.5,vec2(0.5-uBorderRadius),uBorderRadius); float edgeSmooth=0.002; float alpha=1.0-smoothstep(-edgeSmooth,edgeSmooth,d); gl_FragColor=vec4(color.rgb,alpha); }`,
		...
		const img = new Image(); img.crossOrigin = 'anonymous'; img.src = this.image; img.onload = () => { texture.image = img; this.program.uniforms.uImageSizes.value = [img.naturalWidth, img.naturalHeight]; };
	}
	...
	update(scroll: { current: number; last: number }, direction: 'right' | 'left') {
		this.plane.position.x = this.x - scroll.current - this.extra; const x = this.plane.position.x; const H = this.viewport.width / 2;
	...
	onResize({ screen, viewport }: { screen?: ScreenSize; viewport?: Viewport } = {}) {
		...
		if (viewport) { this.viewport = viewport; if ((this.plane.program as any).uniforms.uViewportSizes) { (this.plane.program as any).uniforms.uViewportSizes.value = [this.viewport.width, this.viewport.height]; } }
...
class App {
	...
	constructor(container: HTMLElement, { items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05 }: AppConfig) {
		document.documentElement.classList.remove('no-js'); this.container = container; this.scrollSpeed = scrollSpeed;
		this.scroll = { ease: scrollEase, current: 0, target: 0, last: 0 }; this.onCheckDebounce = debounce(this.onCheck.bind(this), 200);
	...
	createMedias(items: { image: string; text: string }[] | undefined, bend: number, textColor: string, borderRadius: number, font: string) {
		const defaults = Array.from({ length: 10 }).map((_, i) => ({ image: `https://picsum.photos/seed/circ${i}/800/600?grayscale`, text: `Imagen ${i + 1}` }));
		const galleryItems = items && items.length ? items : defaults;
		this.mediasImages = galleryItems.concat(galleryItems);
		this.medias = this.mediasImages.map((data, index) => new Media({
			geometry: this.planeGeometry, gl: this.gl, image: data.image, index, length: this.mediasImages.length, renderer: this.renderer, scene: this.scene,
	...
	onResize() {
		...
		this.viewport = { width, height }; if (this.medias) { this.medias.forEach(media => media.onResize({ screen: this.screen, viewport: this.viewport })); }
	}
	update() {
...
export default function CircularGallery({ items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, onOpen }: CircularGalleryProps) {
	...
	// Lightbox trigger via overlay elements (we add an invisible layer to capture clicks and figure index by x position)
	const [open, setOpen] = useState(false);
	const [current, setCurrent] = useState(0);

	const handleOpen = (idx: number) => { setCurrent(idx); setOpen(true); onOpen && onOpen(idx); };
	const handlePrev = () => setCurrent((c) => (c - 1 + (items?.length || 0)) % (items?.length || 1));
	const handleNext = () => setCurrent((c) => (c + 1) % (items?.length || 1));

	return (
		<div className="w-full h-full relative">
			<div className="w-full h-full overflow-hidden cursor-grab active:cursor-grabbing" ref={containerRef} />
			{/* Invisible click rows to open at approximate index */}
			{(items && items.length) ? (
				<div className="absolute inset-0 grid grid-cols-8 z-[5]">
					{Array.from({ length: 8 }).map((_, i) => (
						<button key={i} className="w-full h-full" onClick={() => handleOpen(Math.floor((i / 8) * items.length))} aria-label="open-image" />
					))}
				</div>
			) : null}
			{open && items && items.length > 0 && (
				<div className="fixed inset-0 z-[1500] bg-black/90 backdrop-blur-md flex items-center justify-center">
					<button className="absolute top-6 right-6 p-3 rounded-full bg-white/15 hover:bg-white/25 transition" onClick={() => setOpen(false)} aria-label="cerrar">
						<Icon name="X" size={20} className="text-white" />
					</button>
					<button className="absolute left-4 md:left-12 p-3 rounded-full bg-white/15 hover:bg-white/25 transition" onClick={handlePrev} aria-label="anterior">
						<Icon name="ChevronLeft" size={24} className="text-white" />
					</button>
					<div className="max-w-5xl w-[90%] aspect-video bg-black rounded-xl overflow-hidden shadow-2xl">
						<img src={items[current].image} alt={items[current].text} className="w-full h-full object-contain" />
					</div>
					<button className="absolute right-4 md:right-12 p-3 rounded-full bg-white/15 hover:bg-white/25 transition" onClick={handleNext} aria-label="siguiente">
