...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  // simple shuffle to avoid duplicates together
...
export default function DomeGallery({
  ...
  useGesture({
    onDragStart: ({ event }) => {
      ...
      if ((evt as any).pointerType === 'touch') evt.preventDefault();
  ...
  const cssStyles = `
    ...
    .item__image { position: absolute; inset: 10px; border-radius: var(--tile-radius, 12px); overflow: hidden; cursor: pointer; backface-visibility: hidden; transition: transform 300ms; pointer-events: auto; }
  `;

  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: cssStyles }} />
      <div ref={rootRef} className="sphere-root relative w-full h-full" style={{ ['--segments-x' as any]: segments, ['--segments-y' as any]: segments } as React.CSSProperties}>
